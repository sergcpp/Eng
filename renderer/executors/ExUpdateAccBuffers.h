...
class ExUpdateAccBuffers final : public FgExecutor {
    ...
    FgBufRWHandle rt_geo_instances_buf_;
    FgBufRWHandle rt_obj_instances_buf_;

    void Execute_HWRT(const FgContext &fg);
    void Execute_SWRT(const FgContext &fg);

  public:
    ExUpdateAccBuffers(const DrawList *&p_list, int rt_index, const FgBufRWHandle rt_geo_instances_buf,
                       const FgBufRWHandle rt_obj_instances_buf)
