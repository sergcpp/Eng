...
void Eng::ExOITScheduleRays::DrawTransparent(const FgContext &fg, const Ren::ImageRWHandle depth_tex) {
    ...
    const Ren::BufferMain &oit_depth_buf_main = storages.buffers.Get(oit_depth_buf).first;
    ren_glBindTextureUnit_Comp(GL_TEXTURE_BUFFER, OITScheduleRays::OIT_DEPTH_BUF_SLOT,
                               oit_depth_buf_main.views[0].second);

    const Ren::BufferMain &ray_counter_buf_main = storages.buffers.Get(ray_counter_buf).first;
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, OITScheduleRays::RAY_COUNTER_SLOT, GLuint(ray_counter_buf_main.buf));
    const Ren::BufferMain &ray_list_buf_main = storages.buffers.Get(ray_list_buf).first;
