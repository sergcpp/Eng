...
void Eng::ExUpdateAccBuffers::Execute_HWRT(const FgContext &fg) {
    ...
    const Ren::BufferHandle rt_obj_instances_buf = fg.AccessRWBuffer(rt_obj_instances_);
    ...
    if (rt_geo_instances.count) {
        ...
        Ren::BufferMain &rt_geo_instances_buf_main = storages.buffers[rt_geo_instances_buf].first;
    ...
    auto &rt_obj_instances_stage_buf = p_list_->rt_obj_instances_stage[rt_index_];

    if (rt_obj_instances.count) {
        const auto &[rt_obj_instances_stage_buf_main, rt_obj_instances_stage_buf_cold] =
            storages.buffers[rt_obj_instances_stage_buf];
        ...
        if (stage_mem) {
            ...
            for (uint32_t i = 0; i < rt_obj_instances.count; ++i) {
                const auto &[acc_main, acc_cold] = storages.acc_structs[rt_obj_instances.data[i].blas];
