...
class ExBuildAccStructures final : public FgExecutor {
    ...
    FgBufHandle rt_tlas_buf_;
    FgBufHandle rt_tlas_build_scratch_buf_;
  ...
  public:
    ExBuildAccStructures(const DrawList *&p_list, int rt_index, const FgBufHandle rt_obj_instances_buf,
                         const AccelerationStructureData *acc_struct_data, Ren::Span<const mesh_t> rt_meshes,
                         const FgBufHandle rt_tlas_buf, const FgBufHandle rt_tlas_scratch_buf)
