...
void Ren::Image_CmdCopyToImage(const ApiContext &api, CommandBuffer cmd_buf, const ImageMain &src_main,
    ...
    glCopyImageSubData(GLuint(src_main.img), GL_TEXTURE_2D, GLint(src_level), GLint(src_x), GLint(src_y), GLint(src_z),
                       GLuint(dst_main.img), GL_TEXTURE_2D, GLint(dst_level), GLint(dst_x), GLint(dst_y), GLint(dst_z),
...
void Ren::Image::Realloc(const int w, const int h, int mip_count, const int samples, const eFormat format,
    ...
    const ImgHandle new_handle = {tex_id, api_->image_counter++};
...
void Ren::Image::InitFromRAWData(const BufferMain *sbuf_main, const BufferCold *sbuf_cold, int data_off,
    ...
    glObjectLabel(GL_TEXTURE, tex_id, -1, name_.c_str());
#endif
    handle_ = {tex_id, api_->image_counter++};
