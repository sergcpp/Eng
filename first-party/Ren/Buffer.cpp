...
bool Ren::UpdateBuffer(const ApiContext &api, const StoragesRef &storages, const BufferHandle dst,
                       const uint32_t dst_offset, const uint32_t data_size, const void *data, const BufferHandle stage,
                       const uint32_t map_offset, const uint32_t map_size, CommandBuffer cmd_buf) {
    if (!data || !data_size) {
        return true;
    }
    ...
    CopyBufferToBuffer(api, storages, stage, map_offset, dst, dst_offset, data_size, cmd_buf);
